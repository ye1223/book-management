...
/* const fetchTableData = async (page: number = 1, limit: number = pageSize, search: boolean = false,) => {
  try {
    if (!search) {
      ...
      const { backendData, length } = await api.book.search(searchValue.value)
      console.log('bbb', backendData)
...
const handleDelete = async (index: number, row: any) => {
  try {
    await axios.delete(`/api/books/del?delId=${row.Id}`).then(res => {
